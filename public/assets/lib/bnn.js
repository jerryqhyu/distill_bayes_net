function bnn(curve_div, train_loss_div, valid_loss_div, progress_div, graph_div) {
	...
	var valid_loss_plotter = Plotter(valid_loss_div, param.loss_domain_x,
		param.loss_domain_y, true, true);
	var progress_plotter = Plotter(progress_div, param.progress_domain_x, param.progress_domain_y,
		false, true);
	var graph_plotter = Plotter(graph_div, [0, 1], [0, 1], false, false);
	...
	function plot_line() {
		var curve_x = [];
		var training_curve = [];
		var sampled_weights = [[net.getLayer(1).sampled_w[0].w[0], net.getLayer(1).sampled_w[1].w[0], net_lib.randn(0, 0) * 0.25]]
		...
		curve_plotter.plot_line(training_curve, {
			color: "darkgreen",
			width: 2,
			opacity: 1,
			id: "#float"
	...
	function plot_weight() {
		...
		var training_sample = [{
			x: net.getLayer(1).sampled_w[0].w[0],
			y: net.getLayer(1).sampled_w[1].w[0],
		}];
		train_loss_plotter.plot_points(training_sample, {
			stroke: "black",
			color: "darkgreen",
			size: 4,
			opacity: 1,
		...
		valid_loss_plotter.plot_points(training_sample, {
			stroke: "black",
			color: "darkgreen",
			size: 4,
			opacity: 1,
	...
	function plot_curve_distribution() {
		...
		for (var net_idx = 0; net_idx < sample_size; net_idx++) {
			sampled_seeds.push([net_lib.seededGaussian(rng), net_lib.seededGaussian(rng), net_lib.seededGaussian(rng) * 0.25]);
		}
	...
	function plot_contours() {
		...
		for (var w_2 = 0, k = 0; w_2 < param.m; w_2++) {
			for (var w_1 = 0; w_1 < param.n; w_1++, k++) {
				train_contour_data[k] = compute_training_loss(dummy_net, inv_x_scale(w_1 *
					param.scaling_factor), inv_y_scale(w_2 * param.scaling_factor))
				valid_contour_data[k] = compute_validation_loss(dummy_net, inv_x_scale(w_1 *
					param.scaling_factor), inv_y_scale(w_2 * param.scaling_factor));
			}
		}
		plot_contour(train_loss_plotter, train_contour_data, train_contour_color,
			train_contour_scale);
		plot_contour(valid_loss_plotter, valid_contour_data, valid_contour_color,
			valid_contour_scale);
	}
	...
	function get_test_loss_from_prediction(avg_prediction) {
		...
		for (var j = 0; j < param.validation_points.length; j++) {
			true_label = Math.sin(param.validation_points[j]) + param.validation_noise[j];
			total_loss += 0.5 * (true_label - avg_prediction[j].y) * (true_label -
				avg_prediction[j].y);
		}
		return total_loss;
	}

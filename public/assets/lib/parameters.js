param = {
	...
	opt_layer3_b: [
        -0.4959376942683454, -0.38383011398703676, 0.1385379213238358,
		0.14474274177948032
    ],
	opt_layer5_w: [
        [
            -0.7608457857739974, -3.807077771966103, 0.15964555671290204,
			0.4316940153554302
        ],
        [
            -0.46029527392239034, 0.499362627227224, 0.26215741516903984,
			0.6087584334472715
        ],
        [
            -0.12003663770954856, -2.070047029632726, -0.8253090796194713, -
			0.5211260899153192
        ],
        [
            1.1497913505525421, 3.1232664390054965, 0.31466460121428946, -
			1.1103448335850838
        ]
    ],
	opt_layer5_b: [
        -0.09954752198229085, -0.20782723076897178, 1.0460388498407667, -
		0.6316497703555812
    ],
	opt_layer7_w: [
        [0.16424348634195116, -0.6067898480353261, -1.0202767210893393, -
			0.7873295687436086]
    ],
	...
	opt_layer7_vb: [0.1996533593619645],
	seeds: [
        [
	...
	seeds_uncertainty: [[0.02370585, 0.03791983, 0.07134828, 0.01213727,
			0.09210071,
...
var divergence_fill_color = d3.scaleLinear().domain([-50, 0]).interpolate(
	function () {
		return d3.interpolateRdYlGn;
	});

var connection_strength_color = d3.scaleLinear().domain([-2.5, 2.5]).interpolate(
	function () {
...
var train_contour_color = d3.scaleLinear().domain([-2.5, 0.05]).interpolate(
	function () {
		return d3.interpolateSpectral;
	});

var train_contour_scale = d3.contours().size([param.n, param.m]).thresholds(d3.range(-
	0.05, 2.5, 0.05));

var valid_contour_color = d3.scaleLinear().domain([-20, 0.5]).interpolate(
	function () {
		return d3.interpolateSpectral;
	});
var valid_contour_scale = d3.contours().size([param.n, param.m]).thresholds(d3.range(-
	0.5, 20, 0.5));
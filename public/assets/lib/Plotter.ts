class Plotter {
    ...
    scalesAreClamped : bool;

    constructor(div: d3.Selection<any>, xDomain: [number, number], yDomain: [number, number], padded = true, clamp = true) {
        this.padding = padded ? 20 : 0;
        this.width = div.node().getBoundingClientRect().width - padding;
        this.height = div.node().getBoundingClientRect().height - padding;
        this.svg = div.append('svg').attr('height', div.node().getBoundingClientRect().width).attr('height', div.node().getBoundingClientRect().height);

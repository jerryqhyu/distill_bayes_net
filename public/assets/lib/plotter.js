function Plotter(div, domain_x, domain_y, padded, clamp) {
    ...
    this.plot_neural_net = function (net) {
        var _this = this;
        var num_layers = net.layers.filter(function (x) { return x.layer_type == 'fc' || x.layer_type == 'variational'; }).length;
        var points = [];
        var texts = [];
        var lines_to_plot = [];
        var num_slots = num_layers + 2;
        var last_idx = 0;
        net.layers.forEach(function (layer, i) {
            layer_width = layer.out_depth;
            if (layer.layer_type === 'fc') {
                layer.filters.forEach(function (column, j) {
                    points.push({
                        x: (last_idx + 1) / num_slots,
                        y: (j + 1) / (layer_width + 1)
                    });
                    column.w.forEach(function (weight, k) {
                ...
                layer.mu.forEach(function (column, j) {
                    points.push({
                        x: (last_idx + 1) / num_slots,
                        y: (j + 1) / (layer_width + 1)
                    });
        ...
        texts.forEach(function (t) {
            _this.svg.append('text').attr('transform', 'translate(' + t.x + ' ,' + t.y + ')').attr('color', 'black').style('text-anchor', 'middle').style('font-size', 30).attr('opacity', 0.5).text('~');
        });
    };
    ...
    this.add_x_axis_label = function (t) {
        this.svg.append('text').attr('transform', 'translate(' + (this.width / 2) + ' ,' + (this.height + 15) + ')').style('text-anchor', 'middle').attr('color', 'grey').attr('opacity', 0.3).text(t);
    };
    this.add_y_axis_label = function (t) {
        this.svg.append('text').attr('transform', 'translate(' + (this.width + 15) + ',' + (this.height / 2) + ')rotate(-90)').style('text-anchor', 'middle').attr('color', 'grey').attr('opacity', 0.3).text(t);
    };
}
